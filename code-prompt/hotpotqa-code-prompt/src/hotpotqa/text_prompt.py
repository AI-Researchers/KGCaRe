...
from.oracle_retriever import get_hotpot_doc_content
...
class TextPrompt:
    ...
    def __call__(self, question, gold_ans, doc):
        """
        ...
        qtype_raw = self.classify_hotpotqa_question(gold_ans)
        ...
        if qtype == QuestionType.YESNO:
            ...
            # print("doc: ", doc)
            # print("yn_task_reminder: ", self.yn_task_reminder)
    ...
    def _create_chain(self, answer_type):
        ...
        # print("list_icl_chat_examples: ", list_icl_chat_examples)
        ...
        # print("end2endqa_chain:: ", end2endqa_chain)
    ...
    def _get_yes_examples(self, train, num_examples):
        # yes_examples = [x for x in train if x["answer"][0][0] == "yes"]
        yes_examples = [x for x in train if x["answer"] == "yes"]
        # print("yes examples:: ", yes_examples)
        return random.sample(yes_examples, num_examples)

    def _get_no_examples(self, train, num_examples):
        # no_examples = [x for x in train if x["answer"][0][0] == "no"]
        no_examples = [x for x in train if x["answer"] == "no"]
        return random.sample(no_examples, num_examples)

    def _get_span_examples(self, train, num_examples):
        # span_examples = [x for x in train if x["answer"][0][0] not in ["yes", "no"]]
        span_examples = [x for x in train if x["answer"] not in ["yes", "no"]]
        # print("span_examples: ", span_examples)
        # print("num_examples=", num_examples)
        return random.sample(span_examples, num_examples)
    
    def classify_hotpotqa_question(self, gold_ans):
        # print("classify qtype called --")
