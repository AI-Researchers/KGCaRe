...
def evaluate(qid2predictions, qid2references):
    ...
    yesno_questions, extractive_questions, conditional_questions = [], [], []

    i = 0
    for _, qid in enumerate(qid2references):
        ...
        if qid2references[qid]:
            if any(ans[0] in ["yes", "no"] for ans in qid2references[qid]):
    ...
    def update_metrics(questions, prefix=""):
        return {
            prefix + "EM": sum(total_em[i] for i in questions) / len(questions) if questions else 0.0,
            prefix + "EM_with_conditions": sum(total_conditional_em[i] for i in questions) / len(questions) if questions else 0.0,
            prefix + "F1": sum(total_f1[i] for i in questions) / len(questions) if questions else 0.0,
            prefix + "F1_with_conditions": sum(total_conditional_f1[i] for i in questions) / len(questions) if questions else 0.0,
        }
...
def run_conditionalqa_evaluation(pred_file, ref_file, question_type="all_types"):
    """Callable function for programmatic evaluation."""
    with open(ref_file, "r", encoding="utf-8") as f:
        ref_data = [ex for ex in json.load(f) if not ex.get("not_answerable", False)]
    ...
    for item in ref_data:
        ...
        if all(ans[0] in ["yes", "no"] for ans in answers):
            if len(answers) == 1:
                qtype = "yesno_conditional" if answers[0][1] else "yesno_single_answer"
            elif len(answers) > 1:
                qtype = "yesno_multi_answer"
        elif any(a[1] for a in answers):
            qtype = "conditional"

        ref_type_map[qid] = qtype
        if question_type == "all_types" or ref_type_map[qid] == question_type:
            filtered_ids.append(qid)

    with open(pred_file, "r", encoding="utf-8") as f:
        pred_data = [json.loads(line.strip()) for line in f]

    qid2pred = {d["id"]: d["answer"] for d in pred_data if d["id"] in filtered_ids}
    qid2ref = {d["id"]: d["answers"] for d in ref_data if d["id"] in filtered_ids}

    return evaluate(qid2pred, qid2ref), ref_type_map, len(filtered_ids)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question_type", default="all_types", choices=[
        "yesno_conditional", "yesno_single_answer", "yesno_multi_answer",
        "extractive", "conditional", "all_types"
    ])
    args = parser.parse_args()

    result, type_map, n = run_conditionalqa_evaluation(
        args.pred_file, args.ref_file, args.question_type
    )

    print(f"Evaluating question type: {args.question_type} (Total = {n})")
    counter = collections.Counter(type_map[qid] for qid in type_map)
    for k in ["yesno_conditional", "yesno_single_answer", "yesno_multi_answer", "extractive", "conditional"]:
