...
# with open('./data/train_FS_sampled_triples.json', 'r') as f:
#     data = json.load(f)

# Function to filter questions by type and randomly select n examples
def select_random_few_shots(data: List[Dict], question_type: str, n: int = 5) -> str:
    """
    Selects and formats n few-shot examples from a dataset based on question type.

    Args:
        data (list): JSON data loaded from train_FS_sampled_triples.json
        question_type (str): Question type filter ('yes/no', 'yes/no_conditional', 'span', 'span_conditional')
        n (int): Number of few-shot examples to return

    Returns:
        str: A formatted string of few-shot examples for prompting
    """
    # Filter examples of the specified question type
    ...
    for example in selected_examples:
        ...
        evidence = ', '.join(example['evidences'])  # Can be used if needed
...
# formatted_output = select_random_few_shots(data, question_type, n)
