...
class KGCaRe(BaseRetriever):
    ...
    def _retrieve(self, query_str: str , topic_entities: List[str]) -> List[NodeWithScore]:
        if not topic_entities:
            topic_entities = self._get_keywords(query_str)
        if not topic_entities:
            raise ValueError("No topic entities found in query.")
    ...
    def _prune_triples(
        ...
        response = self.llm.chat(msgs,stop=["\n<"]).message.content.strip()
        
        flag, retrieve_triples_with_scores = self._clean_triple_prune_output(response)
