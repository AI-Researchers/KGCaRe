...
from hybridrag.data import DATA_ROOT, build_corpus_rows, dataset_paths, default_index_dir, write_json, write_jsonl
from hybridrag.indexing import build_index, make_embeddings
from hybridrag.llm import make_chat_model


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the shared HybridRAG ConditionalQA index.")
    ...
    parser.add_argument(
        "--structured-method",
        choices=["function_calling", "json"],
        default="function_calling",
        help="KG extraction structured output method. Use json for vLLM/open-source ingestion models.",
    )
    parser.add_argument("--structured-retries", type=int, default=2)
    parser.add_argument("--skip-failed-kg-chunks", action="store_true", default=False)
    return parser
...
def ensure_corpus(corpus_path: Path, docs_dir: Path) -> None:
    if corpus_path.exists():
        return
    rows = build_corpus_rows(docs_dir)
    count = write_jsonl(corpus_path, rows)
    write_json(
        dataset_paths("ConditionalQA").metadata_path,
        {
