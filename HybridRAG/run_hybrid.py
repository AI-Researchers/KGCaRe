...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run HybridRAG QA on ConditionalQA.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of structured generation attempts before applying --on-parse-error.",
    )
    parser.add_argument(
        "--on-parse-error",
        choices=["auto", "raise", "unanswerable"],
        default="auto",
        help="How to handle repeated structured-output parse failures. auto raises for OpenAI and records unanswerable for vLLM.",
    )
...
def resolve_parse_error_mode(provider: str, requested: str) -> str:
    if requested != "auto":
        return requested
    return "unanswerable" if provider == "vllm" else "raise"
...
def main() -> None:
    ...
    qa_run_dir = run_dir("ConditionalQA", provider, args.llm_model, args.run_name)
    output_path = args.output_path or (qa_run_dir / "predictions.jsonl")

    if not queries_path.exists():
        raise FileNotFoundError(f"Missing queries file: {queries_path}. Run HybridRAG/prepare_conditionalqa.py first.")
    ...
    with output_path.open(mode, encoding="utf-8") as file_obj:
        for row in tqdm(rows, desc=f"HybridRAG QA {args.llm_model}", unit="q", dynamic_ncols=True):
            if row["id"] in processed:
                continue
            retrieval_query = row.get("composed_question") or f"{row.get('scenario', '')} {row.get('question', '')}".strip()
            ...
            structured: ConditionalQAResponseEnvelope | None = None
            raw_response = ""
            parse_mode = ""
            parse_error = ""
            ...
            if structured is None:
                structured = ConditionalQAResponseEnvelope(
                    response=UnanswerableAnswer(answer_type="unanswerable", rationale=parse_error[:1000])
                )
                raw_response = parse_error
                parse_mode = "parse_error_unanswerable"

            normalized_answer = normalize_eval_answers_for_question_type(
                response_to_eval_answers(structured.response),
                row.get("question_type", "span"),
            )
