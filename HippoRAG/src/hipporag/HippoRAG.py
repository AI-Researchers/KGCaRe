...
from .utils.llm_utils import ConditionalQAModel
...
class HippoRAG:
    ...
    def _extract_question_type(self, question: str) -> str:
        """
        Extract the question type from a query string, e.g. [Question Type: yes/no].
        """
        match = re.search(r"\[Question Type:\s*([^\]]+)\]", question, flags=re.IGNORECASE)
        if match:
            return match.group(1).strip().lower().replace("/", "_")
        return "span"

    def _get_qa_prompt_name(self, question_type: str) -> str:
        dataset = self.global_config.dataset
        specific_prompt = f"rag_qa_{dataset}_{question_type}"
        if self.prompt_template_manager.is_template_name_valid(name=specific_prompt):
            return specific_prompt

        generic_prompt = f"rag_qa_{dataset}"
        if self.prompt_template_manager.is_template_name_valid(name=generic_prompt):
            return generic_prompt

        return "rag_qa_musique"

    def _parse_answer_from_response(self, response_content: str) -> str:
        """
        Extract the final answer from the LLM response, trying JSON structured output first.
        """
        ...
        try:
            ...
            if json_text:
                ...
                validated = ConditionalQAModel.model_validate(parsed)
        ...
        if "Answer:" in text:
            ...
            if "Conditions:" in parts:
                parts = parts.split("Conditions:", 1)[0]
            return parts.strip()
        return text.strip()

    def _extract_json_block(self, text: str) -> str | None:
        """
        Return the first JSON object block found in the text, if any.
        """
        import json
        start = text.find("{")
        if start == -1:
    ...
    def qa(self, queries: List[QuerySolution]) -> Tuple[List[QuerySolution], List[str], List[Dict]]:
        ...
        for query_solution in tqdm(queries, desc="Collecting QA prompts"):
            ...
            question_type = self._extract_question_type(query_solution.question)
            prompt_name = self._get_qa_prompt_name(question_type)
        ...
        for query_solution_idx, query_solution in tqdm(enumerate(queries), desc="Extraction Answers from LLM Response"):
            ...
            pred_ans = self._parse_answer_from_response(response_content)
