...
from ..utils.llm_utils import fix_broken_generated_json, filter_invalid_triples, PROMPT_JSON_TEMPLATE
from ..utils.misc_utils import TripleRawOutput, NerRawOutput
from ..llm.openai_gpt import CacheOpenAI

# Token budgets for extraction — kept separate from QA max_new_tokens
_NER_MAX_TOKENS = 512
_TRIPLE_MAX_TOKENS = 2048

# JSON schema response_format dicts — enforces exact output structure via guided decoding
_NER_RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {"name": "ner_output", "schema": PROMPT_JSON_TEMPLATE["ner"], "strict": False},
}
_TRIPLE_RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {"name": "triple_output", "schema": PROMPT_JSON_TEMPLATE["triples"], "strict": False},
}
...
class OpenIE:
    ...
    def ner(self, chunk_key: str, passage: str) -> NerRawOutput:
        ...
        try:
            ...
            raw_response, metadata, cache_hit = self.llm_model.infer(
                ...
                max_completion_tokens=_NER_MAX_TOKENS,
                response_format=_NER_RESPONSE_FORMAT,
            )
    ...
    def triple_extraction(self, chunk_key: str, passage: str, named_entities: List[str]) -> TripleRawOutput:
        ...
        try:
            ...
            raw_response, metadata, cache_hit = self.llm_model.infer(
                messages=messages,
                max_completion_tokens=_TRIPLE_MAX_TOKENS,
                response_format=_TRIPLE_RESPONSE_FORMAT,
            )
            metadata['cache_hit'] = cache_hit
            if metadata['finish_reason'] == 'length':
                real_response = fix_broken_generated_json(raw_response)
